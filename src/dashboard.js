...
// --- AI Microlearning page logic (moved to top-level so it's available on category navigation) ---
function initMicrolearningPage() {
  ...
  <button class="px-3 py-2 bg-gray-100 text-blue-900 rounded-lg hover:bg-gray-200" id="backToList">Back</button>
    ...
    if (backBtn) backBtn.onclick = () => renderQuizList();
    const resetBtn = document.getElementById('resetQuiz');
    if (resetBtn) resetBtn.onclick = () => startQuiz(q.id);
    const submitBtn = document.getElementById('submitQuiz');
    if (submitBtn) submitBtn.onclick = gradeQuiz;
  }
  ...
  <button class="px-3 py-2 bg-white text-blue-900 rounded-lg border hover:bg-gray-50" id="backToCatalog">Back to Catalog</button>
    ...
    if (retake) retake.onclick = () => startQuiz(q.id);
    const back = document.getElementById('backToCatalog');
    if (back) back.onclick = renderQuizList;
...
function openWeeklyModal(idx, openerEl) {
  ...
  // Social sharing for weekly posts
  const weeklyShare = getWeeklyShareLinks(idx);
  articleSocialShare.innerHTML = `
      <span class="font-semibold mr-2">Share:</span>
      <a href="${weeklyShare.twitter}" target="_blank" rel="noopener" aria-label="Share on Twitter" class="text-blue-500 hover:underline"><svg class="inline w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M22.46 6c-.77.35-1.6.59-2.47.7a4.3 4.3 0 001.88-2.37 8.59 8.59 0 01-2.72 1.04A4.28 4.28 0 0016.11 4c-2.37 0-4.29 1.92-4.29 4.29 0 .34.04.67.11.99C7.69 9.13 4.07 7.38 1.64 4.7c-.37.63-.58 1.36-.58 2.14 0 1.48.75 2.78 1.89 3.54a4.25 4.25 0 01-1.94-.54v.05c0 2.07 1.47 3.8 3.42 4.19-.36.1-.74.16-1.13.16-.28 0-.54-.03-.8-.08.54 1.68 2.12 2.9 3.99 2.93A8.6 8.6 0 012 19.54a12.13 12.13 0 006.56 1.92c7.88 0 12.2-6.53 12.2-12.2 0-.19 0-.38-.01-.57A8.72 8.72 0 0024 4.59a8.5 8.5 0 01-2.54.7z"/></svg></a>
      <a href="${weeklyShare.facebook}" target="_blank" rel="noopener" aria-label="Share on Facebook" class="text-blue-700 hover:underline"><svg class="inline w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M22.675 0h-21.35C.595 0 0 .592 0 1.326v21.348C0 23.408.595 24 1.325 24h11.495v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.797.143v3.24l-1.918.001c-1.504 0-1.797.715-1.797 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116C23.406 24 24 23.408 24 22.674V1.326C24 .592 23.406 0 22.675 0"/></svg></a>
      <a href="${weeklyShare.linkedin}" target="_blank" rel="noopener" aria-label="Share on LinkedIn" class="text-blue-800 hover:underline"><svg class="inline w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.76 0-5 2.24-5 5v14c0 2.76 2.24 5 5 5h14c2.76 0 5-2.24 5-5v-14c0-2.76-2.24-5-5-5zm-11.75 20h-3v-10h3v10zm-1.5-11.25c-.97 0-1.75-.78-1.75-1.75s.78-1.75 1.75-1.75 1.75.78 1.75 1.75-.78 1.75-1.75 1.75zm15.25 11.25h-3v-5.5c0-1.32-.03-3.01-1.84-3.01-1.84 0-2.12 1.43-2.12 2.91v5.6h-3v-10h2.88v1.36h.04c.4-.75 1.38-1.54 2.84-1.54 3.04 0 3.6 2 3.6 4.59v5.59z"/></svg></a>
      <a href="${weeklyShare.reddit}" target="_blank" rel="noopener" aria-label="Share on Reddit" class="text-orange-600 hover:underline"><svg class="inline w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.302 3.438 9.8 8.205 11.385.6.111.82-.261.82-.577 0-.285-.011-1.04-.017-2.04-3.338.726-4.042-1.415-4.042-1.415-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.84 1.236 1.84 1.236 1.07 1.834 2.809 1.304 3.495.997.108-.775.418-1.305.762-1.606-2.665-.304-5.466-1.332-5.466-5.931 0-1.31.469-2.381 1.236-3.221-.124-.303-.535-1.523.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.553 3.297-1.23 3.297-1.23.653 1.653.242 2.873.119 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.803 5.625-5.475 5.921.43.372.823 1.102.823 2.222 0 1.606-.015 2.898-.015 3.293 0 .319.216.694.825.576 4.765-1.587 8.199-6.084 8.199-11.384z"/></svg></a>
    `;
